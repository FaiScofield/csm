    ...
    double *x_out)
{
    ...
    for(k=0; k<K; ++k) {
        ...
        */
        ...
        d_g[0] += qx * C00 + qy * C10;
        d_g[1] += qx * C01 + qy * C11;
        d_g[2] += qx * (px*C00 + py*C01) + qy * (px*C10 + py*C11);
        d_g[3] += qx * ((-py)*C00 + px*C01) + qy * ((-py)*C10 + px*C11);
    ...
    /* q = p - l^2        */
    double q[5] = { p[0]-(l[0]*l[0]),
                    p[1]-(2*l[1]*l[0]),
                    p[2]-(l[1]*l[1]+2*l[0]*l[2]),
                    -(2*l[2]*l[1]),
                    -(l[2]*l[2])    };
    ...
    */
    ...
    M(W, 4, 4);
    gsl_matrix_set_zero(W); gms(W,2,2,1.0); gms(W,3,3,1.0);
    M(x, 4, 1);
