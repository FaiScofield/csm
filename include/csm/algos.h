...
struct sm_params {
    ...
    /** Where to start when iteration begin */
    ...
    */
    ...
    /** Add by Vance, for key frames selection */
    int kf_delta_frame;
    double kf_dist_linear, kf_dist_angular;

    /** Add by Vance, for pose graph optimization */
    int pg_max_iterations;
    int pg_max_frames;

    /** Add by Vance, for iteration visible test */
    int icp_iteration_visible;

    /** Add by Vance, for valid transform evaluation */
    double max_linear_velocity, max_angular_velocity;
    double min_valid_distance, min_valid_angle;
    double corr_fail_perc;
    int use_imu_yaw_as_absolute_rotation;

    /** Add by Vance, copy construction */
    sm_params(){}
    sm_params(const sm_params* other);
};
